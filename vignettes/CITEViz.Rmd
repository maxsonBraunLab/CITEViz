...
In this vignette, we used a 10K cell down-sample of the PBMC dataset from [Hao et. al. 2021](https://doi.org/10.1016/j.cell.2021.04.048).

**Getting started:** The test dataset (in RDS format) for this vignette can be downloaded [here](https://drive.google.com/file/d/1GLRx8KEh_erayt7FhhcWeWR7uyWEVESt/view?usp=sharing).

## Data Export

CITEViz uses interactive data visualization and exploration packages such as [Plotly](https://plotly-r.com/) and [DT](https://rstudio.github.io/DT/). `Plotly` plots can be exported as PNG files, and `DT` datatables
can be copied, printed, or exported as files for downstream analysis.
...
knitr::include_graphics("../inst/app/www/gate_datatable_renaming.png")
```
...
```{r, echo = FALSE, out.width = "100%", out.height = "125%", fig.align="center"}
knitr::include_graphics("../inst/app/www/gating_CD8_T_Cells2.png")
```
...
CITEViz gates contain a rich resource of metadata that can facilitate further downstream analyses. In the tutorial below, B-cell and NK-cell clusters are isolated in CITEViz, then re-imported into [Seurat v4](https://satijalab.org/seurat/) (Hao et al. 2021) for differential expression analysis.
