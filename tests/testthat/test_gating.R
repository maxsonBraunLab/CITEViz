...
test_gate1 <- Gate(counter = as.integer(1), 
                  assay_name = "test1", 
...
test_that("Gate() create correctly formatted Gate object", {
  expect_type(test_gate1@assay_name, "character")
  expect_type(test_gate1@num_input_cells, "integer")
  expect_type(test_gate1@input_cells, "list")
  expect_equal(test_gate1@x_axis, "ADT-A")
  expect_equal(test_gate1@y_axis, "ADT-B")
})
...
test_gate2 <- Gate(counter = as.integer(2), 
                   assay_name = "test2", 
...
test_that("update_gating_df() updated gating dataframes correctly", {
  ...
  expect_equal(test_df$Num_Input_Cells[2], test_gate2@num_input_cells)
  expect_equal(test_df$Num_Input_Cells[1], test_gate1@num_input_cells)
  expect_equal(test_df$Input_X_Coordinates[[1]], test_gate1@input_coords$x)
})
