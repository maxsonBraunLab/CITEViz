...
test_that(desc = "get_data_from_sce retrieves the correct data", {
  ...
  test_data_assays_sce <- get_data_from_sce(category = "assays", 
  ...
  test_data_reduction_sce <- get_data_from_sce(category = "reductions", 
...
counts <- matrix(rpois(30, lambda = 20),nrow = 3, ncol = 10, 
...
test_so[["pca"]] <- Seurat::CreateDimReducObject(embeddings = matrix(rpois(20, lambda = 5),
                                                                     ncol = 2, nrow = 10,
                                                                     dimnames = list(LETTERS[1:10], c("pca_1", "pca_2"))), 
                                                 key = "pca_", 
                                                 assay = "RNA")
test_so[["umap"]] <- Seurat::CreateDimReducObject(embeddings = matrix(rpois(20, lambda = 11), 
                                                                      ncol = 2, nrow = 10,
...
test_that(desc = "get_data_from_seurat retrieves the correct data", {
  test_data_assays_so <- get_data_from_seurat(category = "assays", 
  ...
  test_data_reduction_so <- get_data_from_seurat(category = "reductions", 
                                                 seurat_object = test_so, 
                                                 reduction_name = "pca")
  
          expect_equal(names(test_data_metadata_so), c("orig.ident", "nCount_RNA", 
